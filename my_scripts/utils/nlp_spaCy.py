    ...
    return pd.Series(
        [
                " ".join(
                    ...
                    for token in doc 
                    if HAS_ALNUM.search(token.text) and 
                    # some non-word tokens (e.g. '=') may give false positive in POS filter
                    # we therefore explicitly remove tokens containing zero alphanumeric character
                    not (
                        token.pos_ in exclude_pos or token.tag_ in exclude_tag
        ...
        index=S.index, dtype='string[pyarrow]',
    )